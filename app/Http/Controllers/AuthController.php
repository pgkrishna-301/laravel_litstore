    ...
    public function register(Request $request)
{
    $request->validate([
        ...
        'email' => 'nullable|string|email|max:255', // Allow email to be nullable
        'password' => 'required|string|min:8',
        'profile_image' => 'nullable|image|mimes:jpg,jpeg,png|max:2048', // Validate profile image
    ]);

    // Handle file upload
    $profileImagePath = null;
    if ($request->hasFile('profile_image')) {
        $profileImagePath = $request->file('profile_image')->store('uploads', 'public'); // Store file in 'public/uploads'
    }

    DB::table('user')->insert([
        'name' => $request->name,
        'mobile_number' => $request->mobile_number,
        'email' => $request->email, // Nullable field
        'password' => Hash::make($request->password),
        'profile_image' => $profileImagePath, // Save the file path
    ]);
